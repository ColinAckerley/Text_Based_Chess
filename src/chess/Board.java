...
public class Board {
	...
	void move(String color, String move) throws Exception {
		...
		} else if (board[newRow][newCol] == null && (board[curRow][curCol].toString().equalsIgnoreCase("wP")
				|| board[curRow][curCol].toString().equalsIgnoreCase("bP"))) { // if a piece moves to an empty space
		...
		for (int i = 0; i < SIZE; i++) // Go through the whole board
			for (int j = 0; j < SIZE; j++)
				if (curBoard[i][j] != null)
					...
					if (curBoard[i][j].checkMoveValidity(this, curBoard, i, j, kingRow, kingCol)
							&& !curBoard[i][j].getColor().equalsIgnoreCase(color))
						return true;
		return false;
	}
